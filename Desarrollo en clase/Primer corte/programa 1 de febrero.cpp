...
/* Fecha : 27 de enero de 2022
   ...
   Materia: HPC 6
   Tema: Uso de la clase vector, en multiplicación de matrices 
   		 se le inserta medida de tiempo de ejecución. 
  */
...
int main(int argc, char**argv) {
	...
	/*Se requiere tomar el tiempo de ejecución de la operación de matriz*/
	/**/
	auto inicioTiempo = chrono::high_resolution_clock::now();
	MR = multiMatrix(M1,M2);
	auto finTiempo = chrono::high_resolution_clock::now();
	chrono::duration<double, milli> duracion_ms = finTiempo - inicioTiempo;
	
	cout<<"\n\n\n";
	cout<<"Duración en ms "<<duracion_ms.count()<<"\n";
	// imprimirmatrix(MR);
